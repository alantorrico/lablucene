...
public class RocchioTermSelector extends TermSelector{
	...
	private static boolean weighted = Boolean.parseBoolean(ApplicationSetup.getProperty("rocchio.weighted", "false"));
	...
	public void assignTermWeights(int[] docids, float scores[], QueryExpansionModel QEModel) {
		...
		TermSelector selector = TermSelector.getTermSelector(termSelector, this.searcher);
		...
		for (int i=0; i<effDocuments; i++){
			for (int j=0; j<expTerms[i].length; j++){
				termidWeightMap.adjustOrPutValue(expTerms[i][j].getTerm(), expTerms[i][j].getWeightExpansion(), expTerms[i][j].getWeightExpansion());
		...
		for (String termid : termidWeightMap.keys(new String[0])){
			...
			if (effDocuments>this.EXPANSION_MIN_DOCUMENTS && termidDFMap.get(termid)<this.EXPANSION_MIN_DOCUMENTS
		...
		}else{// normalize the expansion weights by the maximum weight among the expansion terms
			for (ExpansionTerm term : candidateTerms) {
				term.setWeightExpansion(term.getWeightExpansion() / normaliser);
				termMap.put(term.getTerm(), term);
			}
	...
	private void multiWeight(float w, ExpansionTerm[] expansionTerms) {
		for(int i=0; i < expansionTerms.length; i++){
			expansionTerms[i].setWeightExpansion(w * expansionTerms[i].getWeightExpansion());
		}
	}

	@Override
	public String getInfo() {
		return (weighted?"W":"") + getType() +"Roc" +addInfo;
	}
	...
	public void assignTermWeights(String[][] terms, int[][] freqss,
		...
		TermSelector selector = TermSelector.getTermSelector("DFRTermSelector", this.searcher);
		selector.setField(field);
		selector.setMetaInfo("normalize.weights", "false");
		ExpansionTerm[][] expTerms = new ExpansionTerm[effDocuments][];
